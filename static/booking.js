(function () {
  var userID = crypto.randomUUID().replace(/-/g, "").slice(0, 12);
  ...
  function renderMovies() {
    ...
    movies.forEach(function (m) {
      ...
      card.className = "movie-card" + (selectedMovie && selectedMovie.id === m.id ? " selected" : "");
      card.innerHTML =
        "<h3>" + escapeHtml(m.title) + "</h3><p>" +
        m.rows + " rows &times; " + m.seats_per_row + " seats</p>";
      card.addEventListener("click", function () { selectMovie(m); });
  ...
  function renderGrid(seatStatuses) {
    ...
    grid.innerHTML = "";
    var statusMap = {};
    seatStatuses.forEach(function (s) { statusMap[s.seat_id] = s; });
    ...
    for (var r = 0; r < selectedMovie.rows; r++) {
      ...
      for (var s = 1; s <= selectedMovie.seats_per_row; s++) {
        ...
        (function (id) {
          btn.addEventListener("click", function () { holdSeat(id); });
        })(seatID);
  ...
  function renderCheckout() {
    ...
    area.innerHTML =
      '<div class="checkout">' +
      "<h3>Checkout</h3>" +
      '<div class="checkout-info"><span>Seat:</span> ' + escapeHtml(activeSession.seatID) + "</div>" +
      '<div class="checkout-info"><span>Movie:</span> ' + escapeHtml(activeSession.movieID) + "</div>" +
      '<div class="checkout-info"><span>Session:</span> ' + activeSession.sessionID.slice(0, 8) + "&hellip;</div>" +
      ...
      "</div>";
    document.getElementById("btnConfirm").addEventListener("click", confirmSeat);
    document.getElementById("btnRelease").addEventListener("click", releaseSeat);
  }
  ...
  function showCheckoutStatus(msg, type) {
    ...
    area.innerHTML =
      '<div class="checkout">' +
      '<div class="status-msg ' + type + '">' + escapeHtml(msg) + "</div>" +
      "</div>";
    setTimeout(function () {
  ...
  function updateTimer() {
    if (!activeSession) { clearTimerInterval(); return; }
    var el = document.getElementById("timer");
    if (!el) return;
    var remaining = Math.max(0, Math.floor((activeSession.expiresAt - Date.now()) / 1000));
